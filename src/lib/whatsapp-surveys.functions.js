...
export const sendWhatsAppSurvey = createServerFn({ method: "POST" })
    ...
    const { data: account, error: accErr } = await supabase
        ...
        .single();
    if (accErr || !account)
        throw new Error("Cuenta de WhatsApp no válida para esta empresa.");
    ...
    if (options.length < 1)
        throw new Error("La encuesta debe tener al menos 1 opción.");
    ...
    if (s.type === 'INTERACTIVE_LIST') {
        ...
        interactive.action = {
            button: "Ver opciones",
            sections: [{ title: "Opciones", rows: options.map((opt) => ({ id: opt.option_key, title: opt.label })) }]
        };
    }
    ...
    const { data: msgRow, error: msgErr } = await supabase
        ...
        .single();
    if (msgErr)
        throw new Error("Error registrando mensaje.");
    ...
    try {
        ...
        await supabase
            .from("whatsapp_messages")
            .update({ status: "failed", error_code: "send_error" })
            .eq("id", messageId);
        throw err;
    }
});
export const saveSurvey = createServerFn({ method: "POST" })
    ...
    await supabase.from("whatsapp_survey_options").insert(optionsToInsert);
    return { id: survey.id };
});
export const getSurveyStats = createServerFn({ method: "GET" })
    ...
    const { data: responses, error: resErr } = await supabase
        ...
        .eq("survey_id", data.surveyId);
    if (resErr)
        throw new Error("Error cargando respuestas");
