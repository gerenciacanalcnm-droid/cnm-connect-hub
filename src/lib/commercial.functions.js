...
export const listPlans = createServerFn({ method: "GET" }).handler(async () => {
    ...
    if (limits.error)
        throw new Error(limits.error.message);
...
export const duplicatePlan = createServerFn({ method: "POST" })
    ...
    if (feats.data?.length) {
        await context.supabase
            .from("plan_features")
            .insert(feats.data.map((f) => ({ ...f, plan_id: newId })));
    }
    if (lims.data?.length) {
        await context.supabase
            .from("plan_limits")
            .insert(lims.data.map((l) => ({ ...l, plan_id: newId })));
    }
    return { id: newId };
});
export const setPlanFeature = createServerFn({ method: "POST" })
    .middleware([requireSupabaseAuth])
    .inputValidator((v) => z.object({ plan_id: uuid, feature_key: z.string().min(1), included: z.boolean() }).parse(v))
    .handler(async ({ data, context }) => {
    const { error } = await context.supabase
        .from("plan_features")
        .upsert(data, { onConflict: "plan_id,feature_key" });
    if (error)
        throw new Error(error.message);
    return { ok: true };
});
export const setPlanLimit = createServerFn({ method: "POST" })
    ...
    const { error } = await context.supabase
        .from("plan_limits")
        .upsert(data, { onConflict: "plan_id,limit_key" });
    if (error)
        throw new Error(error.message);
    return { ok: true };
});
...
export const listWallets = createServerFn({ method: "GET" })
    ...
    for (const r of (recharges ?? [])) {
        if (!last.has(r.company_id))
            last.set(r.company_id, r.created_at);
    }
...
export const listWalletTransactions = createServerFn({ method: "GET" })
    ...
    let q = context.supabase
        ...
        .limit(300);
    if (data.wallet_id)
        q = q.eq("wallet_id", data.wallet_id);
 ...
 */
export async function applyWalletMovement(supabase, input) {
    ...
    if (up.error)
        throw new Error("Error de concurrencia o de red al actualizar wallet.");
    const metadata = {
    ...
    if (walletId.startsWith("no-wallet-")) {
        ...
        if (existing) {
            ...
            const { data: created, error: createErr } = await context.supabase
                ...
                .single();
            if (createErr)
                throw new Error(`Error creando wallet: ${createErr.message}`);
            walletId = created.id;
        }
    }
    const signed = data.type === "AJUSTE_DEBITO" ? -Math.abs(data.amount) : data.amount;
    const res = await applyWalletMovement(context.supabase, {
        walletId,
        amount: signed,
        units: data.units,
        type: data.type,
...
export const createRechargeRequest = createServerFn({ method: "POST" })
    ...
    if (error)
        throw new Error(error.message);
    return { id: row.id, reviewStatus: "pendiente" };
});
export const reviewRecharge = createServerFn({ method: "POST" })
    ...
    if (data.review_status === "aprobada" && recharge.review_status !== "aprobada") {
        ...
        await applyWalletMovement(context.supabase, {
            walletId,
...
export const trackServiceUsage = createServerFn({ method: "POST" })
    ...
    const { data: tiers, error: tErr } = await context.supabase
        ...
        .order("sort_order", { ascending: true });
    if (tErr)
        throw new Error("Error consultando tarifas");
    ...
    const { data: w } = await context.supabase
        ...
        .maybeSingle();
    if (!w)
        throw new Error(`No existe wallet para el canal ${walletChannel}`);
