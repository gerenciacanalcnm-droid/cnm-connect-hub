...
export const submitWhatsAppTemplateToMeta = createServerFn({ method: "POST" })
    ...
    // 3. Envío Real a Meta
    const response = await fetch(`https://graph.facebook.com/v20.0/${account.business_account_id}/message_templates`, {
        method: "POST",
        headers: {
...
Trace ID: ${metaError.fbtrace_id || 'N/A'}`;
    ...
    const { error: updErr } = await context.supabase
        ...
        .eq("id", data.id);
    if (updErr)
        throw new Error(`Error al actualizar DB: ${updErr.message}`);
