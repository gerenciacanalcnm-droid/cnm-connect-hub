...
function CommPage() {
  ...
  return (
    <AdminPage
      ...
      actions={
        <Button size="sm" onClick={() => toast.success("Configuración guardada")}>
          Guardar
        </Button>
      }
    >
      <div className="grid gap-4 lg:grid-cols-2">
        <Card>
          <CardHeader>
            <CardTitle>Proveedor SMS</CardTitle>
            <CardDescription>Balance, límites y horarios operativos.</CardDescription>
          </CardHeader>
          <CardContent className="grid gap-4">
            <Field label="Proveedor">
              <Input defaultValue={s.provider} />
            </Field>
            <div className="grid grid-cols-2 gap-3">
              <Field label="Timeout (s)">
                <Input type="number" defaultValue={s.timeout} />
              </Field>
              <Field label="Reintentos">
                <Input type="number" defaultValue={s.retries} />
              </Field>
            </div>
            <div className="grid grid-cols-2 gap-3">
              ...
              <Field label="Límite diario">
                <Input type="number" defaultValue={s.dailyLimit} />
              </Field>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <Field label="Horario inicio">
                <Input type="time" defaultValue={s.scheduleStart} />
              </Field>
              <Field label="Horario fin">
                <Input type="time" defaultValue={s.scheduleEnd} />
              </Field>
            </div>
            <div className="flex items-center justify-between rounded-md border border-border p-3">
              <div>
                <p className="text-sm font-medium">Flash SMS</p>
                <p className="text-xs text-muted-foreground">
                  Enviar en modo Flash cuando aplique.
                </p>
              </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Proveedor WhatsApp</CardTitle>
            <CardDescription>Integración con Meta Cloud API.</CardDescription>
          </CardHeader>
          <CardContent className="grid gap-4">
            ...
            <Field label="Webhook">
              <Input placeholder="https://api.smscnm.com/wa/hook" />
            </Field>
            <div className="flex items-center justify-between rounded-md border border-border p-3">
              <div>
                <p className="text-sm font-medium">Activar canal</p>
                <p className="text-xs text-muted-foreground">Habilitar envío por WhatsApp.</p>
              </div>
              <Switch />
            </div>
          </CardContent>
      ...
      <Card className="mt-4">
        <CardHeader>
          ...
          <CardDescription>
            Registro global de proveedores por canal. La conexión real se habilita por integración
            oficial.
          </CardDescription>
        </CardHeader>
        <CardContent>
