...
function EmpresasPage() {
  const { data = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const rows = (await listCompanies()) as Array<{ id: string; name: string; slug: string; status: string; balance: number | null }>;
  ...
  return (
    <AdminPage
      ...
      actions={
        <Button size="sm" onClick={() => toast.info("Próximamente: creación de empresas desde el panel.")}>
