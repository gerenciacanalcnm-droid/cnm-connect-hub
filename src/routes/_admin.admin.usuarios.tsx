...
function UsuariosPage() {
  ...
  const columns = useMemo<ColumnDef<(typeof data)[number], unknown>[]>(() => [
    ...
    { accessorKey: "roles", header: "Roles", cell: (c) => (
      <div className="flex flex-wrap gap-1">
        {c.row.original.roles.map((r) => <Badge key={r} variant="outline">{r}</Badge>)}
      </div>
    ) },
    { accessorKey: "status", header: "Estado", cell: (c) => <StatusBadge status={c.row.original.status} /> },
    { accessorKey: "createdAt", header: "Registrado", cell: (c) => formatDateTime(c.row.original.createdAt) },
  ], []);
  ...
  return (
    <AdminPage
      ...
      actions={
        <Button size="sm" onClick={() => toast.info("Próximamente: invitación de usuarios desde el panel.")}>
          <Plus className="mr-1.5 h-4 w-4" />Invitar usuario
        </Button>
      }
    >
      {isLoading ? (
