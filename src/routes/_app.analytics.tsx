...
export const Route = createFileRoute("/_app/analytics")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Métricas de entrega, costos y rendimiento por operador y país.",
      },
    ],
  }),
    ...
    <div className="space-y-6 px-4 py-6 sm:px-6 lg:px-8">
      ...
      {isEmpty && (
        <Card>
          <CardContent className="p-10 text-center text-sm text-muted-foreground">
            Próximamente: las métricas se llenarán cuando el módulo SMS esté conectado a un
            proveedor.
          </CardContent>
        </Card>
      )}
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <Kpi
          icon={DollarSign}
          label="Costo total"
          value={formatCurrency(kpis.cost, "MXN", "es-MX")}
          trend="+0%"
          tone="nova"
        />
      </div>
        ...
        <Card className="lg:col-span-2">
          ...
          <CardContent className="h-72">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart
                data={data.delivery.map((d: { x: string; y: number }) => ({
                  date: d.x.slice(5),
                  enviados: d.y,
                }))}
              >
                ...
                <Tooltip
                  contentStyle={{
                    borderRadius: 8,
                    border: "1px solid oklch(0.9 0 0 / 0.3)",
                    fontSize: 12,
                  }}
                />
                <Area
            ...
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie
                  ...
                  paddingAngle={2}
                >
                  {data.byOperator.map((_: unknown, i: number) => (
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <Card>
          <CardHeader>
            <CardTitle>Fallos vs. entregas</CardTitle>
          </CardHeader>
          <CardContent className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart
                data={data.delivery.map((d: { x: string; y: number }, i: number) => ({
                  date: d.x.slice(5),
                  enviados: d.y,
                  fallos: data.failures[i]?.y ?? 0,
                }))}
              >
                <CartesianGrid
                  strokeDasharray="3 3"
        ...
        <Card>
          <CardHeader>
            <CardTitle>Costo diario (MXN)</CardTitle>
          </CardHeader>
          <CardContent className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart
                data={data.cost.map((d: { x: string; y: number }) => ({
                  date: d.x.slice(5),
                  costo: d.y,
                }))}
              >
      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          {data.byCountry.map((c: { country: string; value: number }) => (
            <div key={c.country} className="space-y-1">
              ...
              <div className="h-2 overflow-hidden rounded-full bg-muted">
                <div
                  className="h-full rounded-full bg-gradient-to-r from-nova to-primary"
                  style={{ width: `${c.value}%` }}
                />
              </div>
            </div>
    ...
    <Card>
      <CardContent className="p-4">
        <div className="flex items-start justify-between">
          ...
          <span
            className={`flex items-center gap-1 text-xs font-medium ${up ? "text-emerald-600" : "text-destructive"}`}
          >
