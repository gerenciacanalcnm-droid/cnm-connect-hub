                ...
                <div className="flex items-start justify-between gap-2">
                  ...
                  <div className="flex flex-col items-end gap-1 text-xs">
                    <Button
                      ...
                      onClick={() =>
                        setPlans((ps) =>
                          ps.map((x) => (x.id === p.id ? { ...x, featured: !x.featured } : x)),
                        )
                      }
                    >
                      <Star
                        className={`h-3.5 w-3.5 ${p.featured ? "fill-amber-400 text-amber-400" : ""}`}
                      />
                    </Button>
                  </div>
                </div>
              ...
              <CardContent className="space-y-3">
                ...
                <div className="flex items-center justify-between rounded-md border border-border p-2 text-xs">
                  ...
                  <Switch
                    ...
                    onCheckedChange={() =>
                      setPlans((ps) =>
                        ps.map((x) => (x.id === p.id ? { ...x, visible: !x.visible } : x)),
                      )
                    }
                  />
                </div>
                <div className="flex gap-2">
                  ...
                  <Button
                    variant="ghost"
                    size="icon"
                    onClick={() => setPlans((ps) => ps.filter((x) => x.id !== p.id))}
                  >
