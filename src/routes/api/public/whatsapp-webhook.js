                ...
                try {
                    ...
                    if (messages && Array.isArray(messages)) {
                        for (const message of messages) {
                            ...
                            const { data: inboundMsgRow, error: inboundErr } = await supabaseAdmin.from('whatsapp_messages').insert({
                                company_id: account.company_id,
                            ...
                            const inboundMsgId = inboundMsgRow?.id;
                            ...
                            if (message.type === 'interactive' && (message.interactive?.list_reply || message.interactive?.button_reply)) {
                                ...
                                const { data: lastOutbound } = await supabaseAdmin
                                    ...
                                    .limit(1)
                                    .maybeSingle();
                                const surveyId = lastOutbound?.metadata?.survey_id;
                                if (surveyId) {
                                    ...
                                    if (optionId) {
                                        ...
                                        await supabaseAdmin.from('whatsapp_survey_responses').insert({
                                            company_id: account.company_id,
                                            survey_id: surveyId,
                                            option_id: optionId,
                                            contact_id: contact.id,
                                        ...
                                        await processAutomationTrigger(account.company_id, 'SURVEY_RESPONSE', {
                                            phone: from,
                                            survey_id: surveyId,
                            ...
                            await processAutomationTrigger(account.company_id, 'whatsapp_message', { phone: from, text: text, wamid, conversation_id: conversationId, contact_id: contact.id }, wamid);
                            ...
                            const { data: settingsData } = await supabaseAdmin
                                ...
                                .eq('company_id', account.company_id)
                                .maybeSingle();
                            const isNovaActive = settingsData?.status === 'ACTIVO';
                            const isAssigned = conv?.assigned_to != null;
                            const isClosed = conv?.status === 'closed';
                            if (isNovaActive && !isAssigned && !isClosed) {
                                try {
                                    // 1. Buscar Mapa Activo
                                    const { data: activeMap } = await supabaseAdmin.from('conversation_maps')
                                        .select('*')
                                        .eq('company_id', account.company_id)
                                        .eq('status', 'ACTIVO')
                                        .limit(1)
                                    ...
                                    if (activeMap) {
                                        ...
                                        const mapResult = await executeConversationMap(account.company_id, contact.id, conversationId, text, activeMap);
                                        responseText = mapResult.response;
                                    }
                                    else {
                                        // Si no hay mapa, comportamiento estándar de Nova
                                        const novaResp = await generateNovaResponse(account.company_id, contact.id, conversationId, text);
                                        responseText = novaResp.response;
                                    }
                                    if (responseText) {
                                        await internalSendNovaResponse(account.company_id, account.id, from, responseText, conversationId, contact.id);
                                    }
                                }
                                catch (novaErr) {
...
async function internalSendNovaResponse(companyId, accountId, recipient, body, conversationId, contactId) {
    ...
    try {
        ...
        // 4. Envío real a Meta
        const metaResponse = await fetch(`https://graph.facebook.com/v20.0/${account.phone_number_id}/messages`, {
            method: "POST",
            headers: {
