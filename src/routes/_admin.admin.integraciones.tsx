...
function IntPage() {
  ...
  return (
    <AdminPage
      title="Integraciones"
      description="Conecta proveedores externos: SMS, WhatsApp, email, CRM y analytics."
    >
        ...
        {items.map((it) => (
          <Card key={it.id}>
            <CardHeader className="flex-row items-start justify-between space-y-0">
              ...
              <Badge variant="outline" className={TONE[it.status]}>
                {it.status === "connected"
                  ? "Conectado"
                  : it.status === "error"
                    ? "Error"
            ...
            <CardContent className="flex items-center justify-between gap-2">
              <Switch
                checked={it.enabled}
                onCheckedChange={(v) => {
                  setItems((xs) => xs.map((x) => (x.id === it.id ? { ...x, enabled: v } : x)));
                  toast.success(v ? `${it.name} habilitado` : `${it.name} deshabilitado`);
                }}
              />
