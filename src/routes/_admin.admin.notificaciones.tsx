...
const ROWS = [
  { k: "email", label: "Correo", icon: Mail, description: "Notificaciones transaccionales y resúmenes." },
  { k: "push", label: "Push", icon: BellIcon, description: "Notificaciones push del navegador y móvil." },
  { k: "inApp", label: "En plataforma", icon: Monitor, description: "Bandeja de notificaciones dentro del panel." },
...
function NotifPage() {
  ...
  return (
    <AdminPage title="Notificaciones" description="Canales de comunicación con administradores y clientes." actions={<Button size="sm" onClick={() => toast.success("Guardado")}>Guardar</Button>}>
      <Card>
        <CardHeader><CardTitle>Canales</CardTitle><CardDescription>Activa qué canales usa la plataforma para notificar.</CardDescription></CardHeader>
        <CardContent className="divide-y divide-border">
          {ROWS.map(({ k, label, icon: Icon, description }) => (
            <div key={k} className="flex items-center justify-between gap-4 py-3">
              <div className="flex items-center gap-3">
                <div className="grid h-9 w-9 place-items-center rounded-md bg-muted"><Icon className="h-4 w-4" /></div>
                <div>
