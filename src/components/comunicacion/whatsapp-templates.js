...
import { FileText, RefreshCw, Search, CheckCircle2, Clock, AlertCircle, PauseCircle, MoreVertical, ExternalLink, Info, Plus, Send, Eye, Type, MousePointer2, Trash2, Variable } from "lucide-react";
...
import { useWhatsAppTemplates, useSyncWhatsAppTemplates, useWhatsAppAccounts, useSaveWhatsAppTemplate, useSubmitWhatsAppTemplateToMeta } from "@/hooks/use-whatsapp";
    ...
    return (<div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
            ...
            return body.replace(/\{\{(\d+)\}\}/g, (match, number) => {
                ...
                <div className="w-80 p-6 border-l overflow-y-auto space-y-4">
                  <div className="flex flex-col sm:flex-row justify-between items-center border-b pb-2 mb-4">
                    ...
                    {selectedComponent && (<Badge variant="outline" className="text-[10px] bg-slate-100 uppercase font-mono">
                        {selectedComponent}
                      </Badge>)}
                  </div>
                  ...
                  <Form {...form}>
                    <form className="space-y-6">
                      {selectedComponent === "HEADER" && (<div className="space-y-4">
                          <FormField control={form.control} name="headerType" render={({ field }) => (<FormItem>
                          ...
                          {form.watch("headerType") === "TEXT" && (<FormField control={form.control} name="headerText" render={({ field }) => (<FormItem>
                          ...
                          {["IMAGE", "VIDEO", "DOCUMENT"].includes(form.watch("headerType")) && (<div className="p-4 bg-muted rounded-lg border text-xs text-muted-foreground flex items-center gap-2">
                      ...
                      {selectedComponent === "BODY" && (<div className="space-y-4">
                          <FormField control={form.control} name="body" render={({ field }) => (<FormItem>
                              ...
                              <Button type="button" size="sm" variant="outline" className="w-full mt-2 gap-2" onClick={() => {
                    ...
                    setTimeout(() => {
                        ...
                        textarea.setSelectionRange(newPos, newPos);
                          ...
                          <div className="space-y-2 pt-2">
                            ...
                            {(form.watch("body").match(/\{\{\d+\}\}/g) || []).map((v, i) => (<div key={i} className="flex items-center gap-2 bg-slate-50 p-2 rounded border text-sm">
                      ...
                      {selectedComponent === "FOOTER" && (<FormField control={form.control} name="footer" render={({ field }) => (<FormItem>
                      ...
                      {selectedComponent === "BUTTONS" && (<div className="space-y-4">
                          <div className="flex items-center justify-between">
                            ...
                            <Button type="button" size="sm" variant="ghost" className="h-7 text-xs gap-1" disabled={(form.watch("buttons")?.length || 0) >= 10} onClick={() => {
                ...
                form.setValue("buttons", [...current, { type: "QUICK_REPLY", text: "Nueva respuesta" }]);
                          ...
                          <div className="space-y-3">
                            {form.watch("buttons")?.map((button, index) => (<div key={index} className="p-3 bg-slate-50 rounded-lg border space-y-3 relative group">
                                <Button type="button" variant="ghost" size="icon" className="absolute top-1 right-1 h-6 w-6 opacity-0 group-hover:opacity-100 transition-opacity" onClick={() => {
                    const current = form.getValues("buttons") || [];
                    form.setValue("buttons", current.filter((_, i) => i !== index));
                }}>
                                  <Trash2 className="h-3 w-3 text-destructive"/>
                                </Button>
                                <FormField control={form.control} name={`buttons.${index}.type`} render={({ field }) => (<FormItem>
                                    <Select onValueChange={field.onChange} defaultValue={field.value}>
                                      <FormControl>
                                ...
                                <FormField control={form.control} name={`buttons.${index}.text`} render={({ field }) => (<FormItem>
                                    <FormControl><Input {...field} placeholder="Texto del botón" className="h-8 text-xs"/></FormControl>
                                  </FormItem>)}/>
                                {button.type === "URL" && (<FormField control={form.control} name={`buttons.${index}.url`} render={({ field }) => (<FormItem>
                                      <FormControl><Input {...field} placeholder="https://..." className="h-8 text-xs"/></FormControl>
                                    </FormItem>)}/>)}
                                {button.type === "PHONE" && (<FormField control={form.control} name={`buttons.${index}.phoneNumber`} render={({ field }) => (<FormItem>
          ...
          <Button onClick={handleSync} disabled={syncMutation.isPending} variant="outline" className="gap-2">
        ...
        <CardHeader className="pb-3">
          <div className="flex items-center gap-2">
            <div className="relative flex-1 max-w-sm">
              ...
              <Input placeholder="Buscar por nombre o categoría..." className="pl-9" value={search} onChange={(e) => setSearch(e.target.value)}/>
            </div>
          </div>
        </CardHeader>
        <CardContent>
            ...
            <Table>
              ...
              <TableBody>
                {isLoading ? (<TableRow>
                  ...
                  </TableRow>) : (filteredTemplates.map((template) => (<TableRow key={template.id}>
                ...
                setIsPreviewOpen(true);
                            ...
                            {template.status === "draft" && (<DropdownMenuItem className="gap-2 text-emerald-600 focus:text-emerald-700" onClick={() => handleSubmitToMeta(template.id)}>
      ...
      <Dialog open={isPreviewOpen} onOpenChange={setIsPreviewOpen}>
        <DialogContent>
          ...
          <div className="space-y-4 py-4">
            <div className="bg-slate-100 rounded-lg p-6 border border-slate-200 max-w-sm mx-auto w-full relative">
              <div className="bg-white rounded-lg shadow-sm p-4 space-y-2 text-sm border-l-4 border-l-emerald-500">
                {currentTemplate?.header && (<div className="font-bold border-b pb-2 text-slate-800 flex items-center gap-2">
                ...
                {currentTemplate?.footer && (<div className="text-[11px] text-slate-500 pt-2 border-t mt-2">
                    {currentTemplate.footer}
                  </div>)}
              </div>
            </div>
            
            {currentTemplate?.variables && currentTemplate.variables.length > 0 && (<div className="space-y-2">
                <h4 className="text-xs font-semibold uppercase text-muted-foreground tracking-wider">Variables detectadas</h4>
                <div className="flex flex-wrap gap-2">
                  {currentTemplate.variables.map((v) => (<Badge key={v} variant="secondary">{"{{" + v + "}}"}</Badge>))}
                </div>
              </div>)}
          ...
          <DialogFooter>
            {currentTemplate?.status === "draft" && (<Button onClick={() => {
