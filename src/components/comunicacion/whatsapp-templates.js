...
import { FileText, Plus, Type, MousePointer2, Variable, Trash2, ChevronLeft, RefreshCw, Save, Send, Globe, Upload, Loader2 } from "lucide-react";
...
export function WhatsAppTemplates() {
    ...
    const sendToMetaMutation = useMutation({
        ...
        onError: (err) => {
            ...
            toast.error(<div className="flex flex-col gap-1 max-w-[400px]">
    ...
    const syncMutation = useMutation({
        ...
        onSuccess: (res) => {
            ...
            toast(<div className="flex flex-col gap-2 min-w-[300px]">
          ...
          {hasErrors && (<div className="space-y-1.5 mt-1 border-t pt-2">
              ...
              <div className="max-h-[150px] overflow-y-auto space-y-1 pr-1">
                {details.filter((d) => !d.success).map((d, i) => (<div key={i} className="text-[10px] bg-red-50 text-red-700 p-1.5 rounded border border-red-100">
          ...
          {!hasErrors && res.updated > 0 && (<p className="text-[10px] text-emerald-600 font-medium">Todas las plantillas se sincronizaron correctamente.</p>)}
        </div>, { duration: hasErrors ? 10000 : 4000 });
        },
        onError: (err) => toast.error(err.message)
    });
    useEffect(() => {
    ...
    if (!isEditorOpen) {
        ...
        <div className="flex justify-between items-center">
          ...
          <div className="flex gap-3">
            <Button variant="outline" onClick={() => account && syncMutation.mutate({ data: { accountId: account.id } })} disabled={syncMutation.isPending || !account}>
        ...
        {templates.length === 0 ? (<Card>
            ...
            {templates.map((tpl) => (<Card key={tpl.id} className="hover:shadow-md transition-shadow cursor-pointer" onClick={() => { setEditingTemplate(tpl); setIsEditorOpen(true); }}>
                <CardContent className="p-6">
                  <div className="flex justify-between items-start mb-4">
                    <div className="flex gap-2">
                      ...
                      {tpl.external_id && (<Badge variant="outline" className="text-[9px] bg-blue-50/50 border-blue-100 text-blue-600">Meta</Badge>)}
                    </div>
    ...
    return (<div className="flex h-screen bg-slate-100 overflow-hidden">
      ...
      <div className="flex-1 p-12 flex items-center justify-center overflow-y-auto">
        <div className="w-full max-w-sm">
          ...
          <div className="bg-[#E7FFDB] rounded-lg shadow-md w-full p-4 relative space-y-3 border border-slate-200">
            {headerType === "TEXT" && headerText && (<div className="font-bold text-sm border-b border-slate-900/10 pb-2 mb-2 text-slate-900">
                {headerText}
              </div>)}

            {headerType === "IMAGE" && (<div className="w-full aspect-video bg-slate-200 rounded-md flex items-center justify-center overflow-hidden border border-slate-300">
                {(localPreviewUrl || headerText) ? (<img src={localPreviewUrl || headerText} alt="Preview" className="w-full h-full object-cover"/>) : (<span className="text-[10px] text-slate-500 font-bold uppercase">Vista previa de imagen</span>)}
              </div>)}

            {headerType === "VIDEO" && (<div className="w-full aspect-video bg-slate-800 rounded-md flex items-center justify-center overflow-hidden border border-slate-900">
                <div className="w-10 h-10 rounded-full bg-white/20 flex items-center justify-center">
                  <div className="w-0 h-0 border-t-[6px] border-t-transparent border-l-[10px] border-l-white border-b-[6px] border-b-transparent ml-1"/>
                </div>
              </div>)}

            {headerType === "DOCUMENT" && (<div className="w-full p-3 bg-white rounded-md border border-slate-200 flex items-center gap-3">
            ...
            {footer && (<div className="text-[11px] text-slate-500 pt-1 border-t border-slate-900/10 font-medium">
                {footer}
              </div>)}
            
            {buttons.length > 0 && (<div className="space-y-1 mt-3">
                {buttons.map((b, i) => (<div key={i} className="bg-white/80 backdrop-blur-sm text-blue-600 text-sm py-2 rounded-lg border border-slate-200 text-center shadow-sm font-medium hover:bg-white transition-colors flex items-center justify-center gap-2">
      ...
      <div className="w-80 bg-white border-l p-6 overflow-y-auto flex flex-col">
        ...
        {!selectedComponent && (<div className="flex-1 flex items-center justify-center text-center text-slate-400">
            <p className="text-sm">Toca un componente de la izquierda para editarlo</p>
          </div>)}

        {selectedComponent === 'HEADER' && (<div className="space-y-4">
            <div className="space-y-2">
            ...
            {headerType === "TEXT" && (<div className="space-y-2">
                ...
                <Input value={headerText} onChange={(e) => setHeaderText(e.target.value)} placeholder="Ej: ¡Oferta especial!" maxLength={60} className="text-slate-900"/>
              </div>)}

            {headerType === "IMAGE" && (<div className="space-y-4">
                <div className="border-2 border-dashed border-slate-200 rounded-lg p-8 text-center bg-slate-50">
                  <input type="file" ref={fileInputRef} className="hidden" accept="image/*" onChange={handleFileUpload}/>
                  <Button variant="outline" size="sm" className="w-full" disabled={isUploading} onClick={() => fileInputRef.current?.click()}>
                ...
                <div className="space-y-2">
                  ...
                  <Input value={headerText} onChange={(e) => setHeaderText(e.target.value)} placeholder="https://..." className="text-slate-900"/>
                </div>
              </div>)}

            {(headerType === "VIDEO" || headerType === "DOCUMENT") && (<div className="space-y-4">
                <div className="border-2 border-dashed border-slate-200 rounded-lg p-8 text-center bg-slate-50">
                  <input type="file" ref={fileInputRef} className="hidden" accept={headerType === "VIDEO" ? "video/*" : ".pdf,.doc,.docx"} onChange={handleFileUpload}/>
                  <Button variant="outline" size="sm" className="w-full" disabled={isUploading} onClick={() => fileInputRef.current?.click()}>
                    {isUploading ? <Loader2 className="h-4 w-4 animate-spin mr-2"/> : <Upload className="h-4 w-4 mr-2"/>}
        ...
        {selectedComponent === 'BODY' && (<div className="space-y-4">
            <div className="space-y-2">
              <div className="flex justify-between items-center">
              ...
              <Textarea value={body} onChange={(e) => setBody(e.target.value)} className="h-64 text-sm text-slate-900 bg-white border-slate-200 focus:ring-emerald-500" placeholder="Hola {{1}}, tu pedido {{2}} está listo."/>
            </div>
            <Button variant="outline" className="w-full text-xs hover:bg-slate-50 border-slate-200 text-slate-600" onClick={() => {
        ...
        {selectedComponent === 'FOOTER' && (<div className="space-y-4">
            <div className="space-y-2">
              <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Pie de página</label>
              <Input value={footer} onChange={(e) => setFooter(e.target.value)} placeholder="Ej: CNM Digital Media" maxLength={60} className="text-slate-900 bg-white"/>
            </div>
          </div>)}

        {selectedComponent === 'BUTTONS' && (<div className="space-y-4">
            ...
            <div className="space-y-3 mt-4 overflow-y-auto max-h-[400px] pr-2">
              {buttons.map((b, i) => (<div key={i} className="p-4 bg-slate-50 rounded-xl border border-slate-200 shadow-sm space-y-4">
                  <div className="flex justify-between items-center">
                    <Select value={b.type} onValueChange={(val) => {
                  ...
                  <div className="space-y-3">
                    ...
                    setButtons(newButtons);
                }} className="h-9 text-sm text-slate-900 bg-white" placeholder="Escribe el texto..." maxLength={25}/>
                    </div>

                    {b.type === 'URL' && (<div className="space-y-1 animate-in slide-in-from-top-1 duration-200">
                        ...
                        setButtons(newButtons);
                    }} className="h-9 text-sm text-slate-900 bg-white" placeholder="https://example.com"/>
                      </div>)}

                    {b.type === 'PHONE' && (<div className="space-y-1 animate-in slide-in-from-top-1 duration-200">
                        <label className="text-[10px] font-semibold text-slate-400 uppercase">Número de teléfono</label>
                        <Input value={b.phoneNumber} onChange={(e) => {
                        const newButtons = [...buttons];
                        newButtons[i].phoneNumber = e.target.value;
                        setButtons(newButtons);
                    }} className="h-9 text-sm text-slate-900 bg-white" placeholder="+573000000000"/>
                      </div>)}
                  </div>
                </div>))}
            </div>
        ...
        <div className="mt-auto pt-6 border-t space-y-3">
          <Button variant="outline" className="w-full text-slate-600" disabled={saveMutation.isPending} onClick={() => {
            saveMutation.mutate({
                data: {
          ...
          <Button variant="outline" className="w-full text-slate-600 border-emerald-100 hover:bg-emerald-50" disabled={isUploading || !account} onClick={() => fileInputRef.current?.click()}>
            {isUploading ? <Loader2 className="h-4 w-4 animate-spin mr-2"/> : <Upload className="h-4 w-4 mr-2"/>}
            Cargar imagen
          </Button>
          
          <Button className="w-full bg-emerald-600 hover:bg-emerald-700 shadow-sm" disabled={!name || !body || (headerType === 'TEXT' && !headerText) || buttons.some(b => !b.text) || sendToMetaMutation.isPending || (headerType !== 'NONE' && headerType !== 'TEXT' && !headerText)} onClick={async () => {
