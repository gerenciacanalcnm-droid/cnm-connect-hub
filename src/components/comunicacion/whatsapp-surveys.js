...
import { Plus, Trash2, ChevronLeft, Smartphone, Send, Save, Image as ImageIcon, Video, FileText, BarChart3, ArrowRight, UserCheck, Percent, MessageSquare } from "lucide-react";
...
// --- Helpers ---
const replaceVariables = (text, examples = { "1": "Juan", "2": "CNM" }) => {
    if (!text)
        return text;
...
export function WhatsAppSurveys() {
    ...
    // Form State
    const [type, setType] = useState('INTERACTIVE_LIST');
        ...
        <div className="w-80 bg-white border-r flex flex-col shadow-sm">
          ...
          <div className="flex-1 overflow-y-auto p-5 space-y-6">
            <section className="space-y-3">
              ...
              <Input value={title} onChange={e => setTitle(e.target.value)} placeholder="Nombre interno de la encuesta" className="bg-white"/>
            </section>

            <section className="space-y-3">
              <label className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Contenido (Meta compatible)</label>
              
              <Select value={headerType} onValueChange={(v) => setHeaderType(v)}>
                <SelectTrigger className="bg-white border-slate-200">
              ...
              {headerType === 'TEXT' && (<Input value={headerText} onChange={e => setHeaderText(e.target.value)} placeholder="Texto de la cabecera (60 chars)" maxLength={60}/>)}

              {['IMAGE', 'VIDEO', 'DOCUMENT'].includes(headerType) && (<Input value={headerUrl} onChange={e => setHeaderUrl(e.target.value)} placeholder="URL del archivo multimedia"/>)}

              <Textarea value={question} onChange={e => setQuestion(e.target.value)} placeholder="Cuerpo del mensaje (soporta {{1}}, {{2}})" className="min-h-[100px] bg-white resize-none" maxLength={1024}/>

              <Input value={footer} onChange={e => setFooter(e.target.value)} placeholder="Pie de página (opcional, 60 chars)" maxLength={60}/>

              {type === 'INTERACTIVE_LIST' && (<Input value={listButtonText} onChange={e => setListButtonText(e.target.value)} placeholder="Texto del botón de lista (20 chars)" maxLength={20}/>)}
            </section>
            ...
            <section className="space-y-3">
              ...
              <div className="space-y-2">
                {options.map((opt, i) => (<div key={i} className="flex gap-2 group">
                    <Input value={opt.label} onChange={e => {
                    const ns = [...options];
                    ns[i].label = e.target.value;
                    setOptions(ns);
                }} placeholder={`Opción ${i + 1}`} className="flex-1 bg-white h-9 text-sm" maxLength={24}/>
                    <Button variant="ghost" size="icon" onClick={() => removeOption(i)} className="h-9 w-9 text-slate-300 hover:text-red-500">
                      <Trash2 className="h-4 w-4"/>
                    </Button>
                  </div>))}
              </div>

              <Button variant="outline" className="w-full h-9 border-dashed text-xs border-slate-300 text-slate-500 hover:border-blue-400 hover:text-blue-600" onClick={addOption}>
          ...
          <div className="w-[320px] bg-white rounded-lg shadow-2xl border border-slate-300 overflow-hidden">
            ...
            <div className="p-3 space-y-4 min-h-[450px] relative">
              <div className="bg-white rounded-lg shadow-[0_1px_0.5px_rgba(0,0,0,0.13)] max-w-[90%] overflow-hidden border border-slate-100/50">
                {/* Cabecera Multimedia */}
                {headerType !== 'NONE' && (<div className="w-full bg-slate-50 border-b border-slate-100 flex items-center justify-center min-h-[120px]">
                    {headerType === 'TEXT' ? (<div className="p-3 w-full font-bold text-slate-800 text-xs">
                        {headerText || "TEXTO DE CABECERA"}
                      </div>) : headerType === 'IMAGE' ? (<div className="flex flex-col items-center text-slate-400">
                        <ImageIcon className="h-8 w-8 mb-2 opacity-50"/>
                        <span className="text-[10px] font-medium">[IMAGEN]</span>
                      </div>) : headerType === 'VIDEO' ? (<div className="flex flex-col items-center text-slate-400">
                        <Video className="h-8 w-8 mb-2 opacity-50"/>
                        <span className="text-[10px] font-medium">[VIDEO]</span>
                      </div>) : (<div className="flex flex-col items-center text-slate-400">
                ...
                <div className="p-3">
                  ...
                  {footer && (<div className="text-slate-400 text-[11px] mt-2 border-t border-slate-50 pt-1">
                      {footer}
                    </div>)}
                </div>
                ...
                <div className="border-t border-slate-100">
                  {type === 'INTERACTIVE_LIST' ? (<div className="flex items-center justify-center p-2.5 text-blue-500 font-bold text-[13px] hover:bg-slate-50 cursor-pointer gap-2">
                      <Send className="h-3.5 w-3.5"/>
                      {listButtonText || "Ver opciones"}
                    </div>) : (<div className="space-y-px bg-slate-100">
                      {options.filter(o => o.label).map((opt, i) => (<div key={i} className="bg-white p-2.5 text-blue-500 text-[13px] text-center font-semibold hover:bg-slate-50 cursor-pointer">
                          {opt.label}
                        </div>))}
                    </div>)}
                </div>
              ...
              {type === 'INTERACTIVE_LIST' && (<div className="absolute inset-x-0 bottom-0 bg-white/95 backdrop-blur-sm border-t border-slate-200 animate-in slide-in-from-bottom-full duration-500 z-20">
                  ...
                  <div className="max-h-[220px] overflow-y-auto">
                    {options.filter(o => o.label).map((opt, i) => (<div key={i} className="px-4 py-3 border-b border-slate-50 flex items-center gap-3 hover:bg-slate-50">
                        <div className="w-4 h-4 rounded-full border border-slate-300"/>
                        <div className="text-slate-700 text-sm">{opt.label}</div>
                      </div>))}
                    {options.filter(o => o.label).length === 0 && (<div className="px-4 py-8 text-center text-slate-400 text-xs italic">
    ...
    // --- Render Dashboard ---
    return (<div className="p-8 space-y-8 max-w-[1200px] mx-auto">
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-3">
          <Button onClick={() => {
            ...
            setQuestion("");
            setFooter("");
            setOptions([{ label: "", option_key: "option_1" }, { label: "", option_key: "option_2" }]);
        ...
        <Card className="bg-blue-600 border-none shadow-lg shadow-blue-200">
          <CardContent className="p-6">
            ...
            <div className="text-3xl font-bold text-white mb-1">
              {(stats?.total ?? 0) > 0 ? `${Math.round(((stats?.stats ?? []).reduce((acc, curr) => acc + curr.count, 0) / (stats?.total ?? 1)) * 100)}%` : "0%"}
            </div>
        ...
        <Card className="bg-white border-slate-200 shadow-sm">
          <CardContent className="p-6">
            ...
            <div className="text-3xl font-bold text-slate-900 mb-1">
              {stats?.stats.reduce((acc, curr) => acc + curr.count, 0) || 0}
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-1 space-y-4">
          ...
          <div className="space-y-2">
             <Card className={cn("cursor-pointer transition-all hover:border-blue-300", selectedSurveyId === "SURVEY_DEMO" ? "border-blue-500 bg-blue-50/30" : "border-slate-100")} onClick={() => setSelectedSurveyId("SURVEY_DEMO")}>
        ...
        <div className="lg:col-span-2">
          {selectedSurveyId ? (<Card className="border-slate-200 bg-white min-h-[400px]">
              ...
              <CardContent className="p-8">
                {isLoadingStats ? (<div className="flex flex-col items-center justify-center py-20 text-slate-400 italic">
                    <div className="animate-spin h-6 w-6 border-2 border-blue-500 border-t-transparent rounded-full mb-4"/>
                    Cargando datos reales...
                  </div>) : stats && stats.stats.length > 0 ? (<div className="space-y-6">
                    {stats.stats.map((opt, i) => (<div key={i} className="space-y-2">
                        ...
                        <div className="w-full bg-slate-100 rounded-full h-2 overflow-hidden">
                          <div className="bg-blue-600 h-full rounded-full transition-all duration-1000" style={{ width: `${opt.percentage}%` }}/>
                        </div>
                      </div>))}
            ...
            </Card>) : (<div className="h-full border-2 border-dashed border-slate-200 rounded-xl flex flex-col items-center justify-center p-12 text-center text-slate-400">
