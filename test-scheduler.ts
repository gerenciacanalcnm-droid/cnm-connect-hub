...
import { executePendingSchedules } from "./src/lib/sms-schedule.server";
...
async function testScheduler() {
  ...
  if (err1) console.error("Error creando sch1:", err1.message);
  else console.log("Creada sch1:", sch1.id);
  ...
  if (err2) console.error("Error creando sch2:", err2.message);
  else console.log("Creada sch2:", sch2.id);

  console.log("\n--- EJECUTANDO SCHEDULER ---");
  const result = await executePendingSchedules(supabaseAdmin);
  console.log("Resultado:", JSON.stringify(result, null, 2));

  const { data: finalSch1 } = await supabaseAdmin.from("sms_schedules").select("status, executed_at, actual_cost").eq("id", sch1.id).single();
  const { data: finalSch2 } = await supabaseAdmin.from("sms_schedules").select("status").eq("id", sch2.id).single();

  console.log("\n--- VERIFICACIÓN ---");
  console.log("Estado final Sch1:", finalSch1?.status, "| Ejecutado:", !!finalSch1?.executed_at);
  console.log("Estado final Sch2:", finalSch2?.status);

  console.log("\n--- TEST 3: Saldo Insuficiente ---");
  const { data: sch3 } = await supabaseAdmin.from("sms_schedules").insert({
    company_id: companyId,
    user_id: userId,
    recipients: Array(20000).fill("3000000000"), 
  ...
  if (sch3) {
    await executePendingSchedules(supabaseAdmin);
